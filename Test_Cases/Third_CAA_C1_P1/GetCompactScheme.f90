...
!! Working TestCase
    ...
    INTERFACE CompactScheme
        MODULE PROCEDURE CompactScheme_NoBC
        MODULE PROCEDURE CompactScheme_Correction
    END INTERFACE CompactScheme

    CONTAINS

    SUBROUTINE CompactScheme_NoBC(  iMin            ,&
                                    iMax            ,&
                                    bMin            ,&
                                    bMax            ,&
                                    E               ,&
    ...
    IF (DS == 6) THEN
        ...
        LowerDiag(i, 1,  3)  = 0.0_rDef 
        LowerDiag(i, 2,  1)  = 0.0_rDef 
        LowerDiag(i, 2,  2)  = 3.0_rDef 
        LowerDiag(i, 2,  3)  = 0.0_rDef 
        LowerDiag(i, 3,  1)  = 0.0_rDef 
        LowerDiag(i, 3,  2)  = 0.0_rDef 
        LowerDiag(i, 3,  3)  = 3.0_rDef 
        ...
        DO j = bMin, bMax 
            i           = iMin
            RHS(i, j)   = ((-17.0_rDef/6.0_rDef)*E(i, j)        +       &
                                        1.5_rDef*E(i + 1, j)    +       &
                                        1.5_rDef*E(i + 2, j)    -       &
                               1.0_rDef/6.0_rDef*E(i + 3, j))/delta_x
            DO i = iMin + 1, iMax - 1
    ...
    ELSEIF ((DS == 7)) THEN
        ...
        MainDiag(i, 1,  1)  = 0.5_rDef + 0.5_rDef
        MainDiag(i, 1,  2)  = 0.0_rDef 
        MainDiag(i, 1,  3)  = 0.0_rDef 
        MainDiag(i, 2,  1)  = 0.0_rDef 
        MainDiag(i, 2,  2)  = 0.5_rDef + 0.5_rDef 
        MainDiag(i, 2,  3)  = 0.0_rDef 
        MainDiag(i, 3,  1)  = 0.0_rDef 
        MainDiag(i, 3,  2)  = 0.0_rDef 
        MainDiag(i, 3,  3)  = 0.5_rDef + 0.5_rDef 
        ...
        DO i = iMin + 1, iMax - 1
            LowerDiag(i, 1,  1)  = 1.0_rDef*(a/(1.0_rDef - a))  
            LowerDiag(i, 1,  2)  = 0.0_rDef                     
            LowerDiag(i, 1,  3)  = 0.0_rDef                     
            LowerDiag(i, 2,  1)  = 0.0_rDef                     
            LowerDiag(i, 2,  2)  = 1.0_rDef*(a/(1.0_rDef - a))  
            LowerDiag(i, 2,  3)  = 0.0_rDef                     
            LowerDiag(i, 3,  1)  = 0.0_rDef                     
            LowerDiag(i, 3,  2)  = 0.0_rDef                     
            LowerDiag(i, 3,  3)  = 1.0_rDef*(a/(1.0_rDef - a))  
            ...
            UpperDiag(i, 1,  1)  = 1.0_rDef*(a/(1.0_rDef - a))
            UpperDiag(i, 1,  2)  = 0.0_rDef 
            UpperDiag(i, 1,  3)  = 0.0_rDef 
            UpperDiag(i, 2,  1)  = 0.0_rDef 
            UpperDiag(i, 2,  2)  = 1.0_rDef*(a/(1.0_rDef - a)) 
            UpperDiag(i, 2,  3)  = 0.0_rDef 
            UpperDiag(i, 3,  1)  = 0.0_rDef 
            UpperDiag(i, 3,  2)  = 0.0_rDef 
            UpperDiag(i, 3,  3)  = 1.0_rDef*(a/(1.0_rDef - a)) 
        END DO
        ...
        DO j = bMin, bMax 
            i           = iMin
            RHS_B(i, j) =  (-2.748875_rDef*E(i      , j)        +&
                            6.9122653_rDef*E(i + 1  , j)        -&
                            8.5709877_rDef*E(i + 2  , j)        +&
                           7.29077104_rDef*E(i + 3  , j)        -&
                           3.93792558_rDef*E(i + 4  , j)        +&
                           1.22040498_rDef*E(i + 5  , j)        -&
                          0.165322497_rDef*E(i + 6  , j))/delta_x

            RHS_F(i, j) = -(2.151125_rDef*E(i      , j)        -&
                            5.087735_rDef*E(i + 1  , j)        +&
                            6.429012_rDef*E(i + 2  , j)        -&
                            6.042892_rDef*E(i + 3  , j)        +&
                            3.562074_rDef*E(i + 4  , j)        -&
                            1.179594_rDef*E(i + 5  , j)        +&
                            0.168011_rDef*E(i + 6  , j))/delta_x
    ...
    ELSEIF (DS == 8) THEN
        ...
        DO j = bMin, bMax 
            i = iMin
            dEdX(i, j) = (RHS_B(i, j) + RHS_F(i, j))*0.5_rDef
    ...
    END SUBROUTINE CompactScheme_NoBC

    SUBROUTINE CompactScheme_Correction(iMin            ,&
                                        iMax            ,&
                                        bMin            ,&
                                        bMax            ,&
                                        E               ,&
    ...
    END SUBROUTINE CompactScheme_Correction
