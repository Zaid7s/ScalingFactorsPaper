...
! Initialize E such that it is easier to calculate dEdX
    ...
    DO nTau = 1, nStages
        DO i = iMin, iMax
            ...
            E(i, 1) = Inv_Jac_Curv(i)*(rho*Contra_U)
            E(i, 2) = Inv_Jac_Curv(i)*(rho*u*Contra_U+dZidX(i)*p)
            E(i, 3) = Inv_Jac_Curv(i)*(Contra_U*(E_tot+p) - dZidt(i)*p)

            S(i, 1) = AreaFac*Inv_Jac_Curv(i)*rho*u
            S(i, 2) = AreaFac*Inv_Jac_Curv(i)*rho*u*u
            S(i, 3) = AreaFac*Inv_Jac_Curv(i)*u*(E_tot+p)
        END DO
        ...
        DO j = bMin, bMax
            DO i = iMin, iMax
                dQdTau(i, j) = (-(dQdT(i, j) + dEdX(i, j) - S(i, j)) &
                                + Dis(i, j)/delta_tau)*(2.0_rDef/3.0_rDef)
                !dQdTau(i, j) = (-(dQdT(i, j) + dEdX(i, j) - S(i, j)) &
                !                + Dis(i, j)/delta_tau)*(3.0_rDef/5.0_rDef)
            END DO
        END DO
