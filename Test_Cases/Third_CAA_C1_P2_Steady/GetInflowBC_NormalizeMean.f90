MODULE GetInflowBC_NormalizeMean
!! Working TestCase
    ...
    IMPLICIT NONE
    PRIVATE
    PUBLIC:: InflowBC_NormalizeMean

    INTEGER, PARAMETER:: rDef = REAL64

    CONTAINS
         
    SUBROUTINE InflowBC_NormalizeMean(    iMin            ,&
                                        iMax            ,&
                                        bMin            ,&
                                        bMax            ,&
                                        E               ,&
    ...
    REAL(KIND = rDef), INTENT(OUT) ::   A_S_BC          ,&
                                        A_Plus_BC       ,&
                                        A_Minus_BC
    
    REAL(KIND = rDef) ::    A_S_noBC        ,&
                            A_Plus_noBC     ,&
                            A_Minus_noBC    ,&
                            p_T             ,&
                            rho_T           ,&
...
! This calculates the inflow values for Second, Fourth, Sixth, and RDRP. 
    ...
    ELSE IF (DS == 4) THEN  ! RDRP
        ...
        dQdT(1) = rho_T 
        dQdT(2) = rho(iMin)*u_t+u(iMin)*rho_t 
        dQdT(3) = gm1*p_T+0.5_rDef*(rho(iMin)*u(iMin)*u_T + &
                                     rho(iMin)*u(iMin)*u_T + &
                                       u(iMin)*u(iMin)*rho_t)
    ...
    END SUBROUTINE InflowBC_NormalizeMean

END MODULE GetInflowBC_NormalizeMean
